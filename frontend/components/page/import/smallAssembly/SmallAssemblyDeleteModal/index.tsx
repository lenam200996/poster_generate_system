...
const SmallAssemblyDeleteModal = (props: Props) => {
  const apiClient = useApiClient()
  const [complete, setComplete] = useState<boolean>(false)

  const handleOnClickDelete = async () => {
    try {
      await apiClient.documentFillersApi.documentFillerControllerDelete(
        props.id,
      )
      setComplete(true)
    } catch (error) {
...
export default SmallAssemblyDeleteModal
