...
interface Props {
  item: Pick<Header, "code" | "name" | "media" | "plate">
  onChange?: (name: string, value: string | string[] | Header["plate"]) => void
}
...
const HeaderForm = (props: Props) => {
  ...
  return (
    <table className='w-full text-sm text-content-default-primary'>
      <tbody>
        {["/import/header/edit/[id]", "/import/header/detail/[id]"].includes(
          router.pathname,
        ) && (
          <tr className='border-b border-divider-accent-primary'>
            ...
            <td className='flex items-center py-3'>{props.item.code}</td>
          </tr>
        )}
        <tr className='border-b border-divider-accent-primary'>
          <th
