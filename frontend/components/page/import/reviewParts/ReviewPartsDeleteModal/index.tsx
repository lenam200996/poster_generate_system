...
const ReviewPartsDeleteModal = (props: Props) => {
  const apiClient = useApiClient()
  const [complete, setComplete] = useState<boolean>(false)

  const handleOnClickDelete = async () => {
    try {
      await apiClient.documentEvaluationsApi.documentEvaluationControllerDeleteDocumentEvaluation(
        props.id,
      )
      setComplete(true)
    } catch (error) {
...
export default ReviewPartsDeleteModal
