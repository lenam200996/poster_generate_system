...
const ImportIndexHeaderDeleteModal = (props: Props) => {
  const apiClient = useApiClient()
  const [complete, setComplete] = useState<boolean>(false)

  const handleOnClickDelete = async () => {
    try {
      await apiClient.thumbIndexApi.thumbIndexControllerDelete(props.id)
      setComplete(true)
    } catch (error) {
      console.error(error)
    }
...
export default ImportIndexHeaderDeleteModal
